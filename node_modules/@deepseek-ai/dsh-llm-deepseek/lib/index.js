...
function apply(ctx, config) {
	...
	options();
	/**
	* Round-robin over the DeepSeek credential reference and the numbered
	* companions (`<ref>_2` … `<ref>_9`) the web Models page stores — consecutive
	* requests, and every retry the recovery policy schedules, take the next
	* configured key.
	*/
	...
	const resolveApiKey = async (connection) => {
		...
		const rotated = await rotateApiKey(ref, resolveOne, (candidateRef, value) => assertUsableApiKey(value, "llm-deepseek", candidateRef), (candidateRef, error) => {
			ctx.logger.warn("llm-deepseek: skipping an unusable API key from %s during rotation", candidateRef);
	...
	/**
	* Minimal chat-completions probe for the Models page connectivity test: one
	* one-token reply against the stored (or drafted) endpoint, using the draft
	* key when the page supplies one and the route's own credential otherwise.
	*/
	ctx.llm.registerModelProbe(NS, async (request, signal) => {
		if (typeof request.model !== "string" || request.model.length === 0) throw new LlmError("model probe needs a model id", "INVALID_PROBE");
		const connection = options();
		const baseURL = (request.baseURL === void 0 || request.baseURL.length === 0 ? connection.baseURL : request.baseURL).replace(/\/+$/u, "");
		let apiKey;
		if (request.apiKey !== void 0 && request.apiKey.length > 0) {
			assertUsableApiKey(request.apiKey, "llm-deepseek", connection.apiKeyEnv);
			apiKey = request.apiKey;
		} else apiKey = await resolveApiKey(connection);
		...
		try {
			...
			if (signal?.aborted) throw new LlmError("model probe aborted by caller", "ABORTED", { cause: error });
		...
		if (!response.ok) {
			...
			throw new LlmError(`${url} answered ${response.status}${detail.length > 0 ? `: ${detail}` : ""}${response.status === 401 || response.status === 403 ? "; check the API key" : response.status === 404 ? "; check the model ID" : ""}`, "PROBE_FAILED");
		}
		/* The reply body carries nothing the caller needs; release it without buffering. */
