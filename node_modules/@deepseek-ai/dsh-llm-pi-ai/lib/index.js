...
* itself discloses — the two spellings the wire has long agreed on
* (`context_window`, `context_length`), the field names self-hosted servers
* and gateways actually ship (vLLM's `max_model_len`, LM Studio's
* `max_context_length`, Novita's `context_size`, LiteLLM's
* `max_input_tokens`), and the nested slots OpenRouter's `top_provider` and
* `metadata` blobs use. An entry that discloses nothing still names an id,
* and an id the installed catalog knows contributes its own nominal context
* window — the best a bare listing can offer.
*
...
/**
* The wire protocols a draft completion probe can speak. This mirrors the
* protocol table minus the lazy API implementations: a probe is one hand-built
* POST whose shape each protocol fully determines.
*/
const PROBE_PROTOCOLS = new Set(["openai-completions", "openai-responses", "anthropic-messages"]);
/**
* The endpoint one probe POST targets, following the same base-URL convention
* as the catalog: OpenAI-compatible bases already carry their version prefix,
* while the Anthropic messages path is appended here.
* @param baseURL - the drafted or stored endpoint root.
* @param api - the wire protocol the endpoint speaks.
* @returns the absolute probe URL.
*/
function probeUrl(baseURL, api) {
...
/**
* The minimal completion body one protocol's probe sends: a one-token reply to
* a single user turn, so a connectivity check costs essentially nothing.
* @param api - the wire protocol the endpoint speaks.
...
/**
* Send one minimal completion probe against a draft endpoint and model.
* @param request - the model id, endpoint, protocol, and one-shot credential to use.
* @param storedProfile - Host-owned headers, lazy credential resolution, and
*   the stored/catalog endpoint facts for the named route. It is read only on
*   the path that reaches the network; the credential is resolved only when the
*   draft carries none.
* @returns nothing; reaching a 2xx reply is the probe's whole result. Latency
*   is measured by the caller.
* @throws LlmError when the protocol has no probe this build can send, the
*   endpoint refuses or fails the request, or the caller aborted.
*/
async function probeModel(request, storedProfile) {
	const stored = storedProfile?.();
	const api = request.api ?? stored?.api ?? "openai-completions";
	if (!PROBE_PROTOCOLS.has(api)) throw new LlmError(`pi-ai protocol "${api}" has no completion probe this build can send; use the model listing to check this endpoint instead`, "PROBE_UNSUPPORTED");
	const baseURL = request.baseURL ?? stored?.baseURL;
	if (baseURL === void 0 || baseURL.length === 0) throw new LlmError(`pi-ai ships no endpoint for provider "${request.provider ?? ""}"; set a baseURL to probe this provider`, "PROBE_FAILED");
	const supplied = request.apiKey ?? await stored?.resolveApiKey?.();
	const apiKey = supplied === void 0 ? void 0 : usableProbeKey(supplied);
	const url = probeUrl(baseURL, api);
	const headers = new Headers(stored?.headers === void 0 ? void 0 : Object.entries(stored.headers));
	headers.set("accept", "application/json");
	...
	for (const [name, value] of Object.entries(attributionHeaders())) headers.set(name, value);
	/* One retry shape: an OpenAI-compatible endpoint that renamed the output cap
	(o-series `max_completion_tokens`) answers the classic spelling with a 400,
	so that one rejection is retried with the newer spelling before reporting. */
	const attempts = api === "openai-completions" ? [probeBody(api, request.model), {
		model: request.model,
		messages: [{ role: "user", content: "ping" }],
		max_completion_tokens: 1,
		stream: false
	}] : [probeBody(api, request.model)];
	let deferredFailure;
	for (let attempt = 0; attempt < attempts.length; attempt++) {
		...
		try {
			...
			if (request.signal?.aborted) throw new LlmError("model probe aborted by caller", "ABORTED", { cause: error });
			throw new LlmError(`could not reach ${url}`, "PROBE_FAILED", { cause: error });
		}
		if (response.ok) {
			/* The reply body carries nothing the caller needs; release it without
			buffering so a chatty endpoint cannot turn a probe into a transfer. */
			await response.body?.cancel().catch(() => {});
			return;
		}
		const detail = await response.text().then((text) => text.replace(/\s+/gu, " ").trim().slice(0, 300)).catch(() => "");
		const failure = new LlmError(`${url} answered ${response.status}${detail.length > 0 ? `: ${detail}` : ""}${response.status === 401 || response.status === 403 ? "; check the API key" : ""}`, "PROBE_FAILED");
		if (attempt + 1 < attempts.length && response.status === 400) {
...
function apply(ctx, config) {
	...
	*/
	...
	profiles();
	/**
	* Round-robin over the profile's primary credential reference and the
	* numbered companions (`<ref>_2` … `<ref>_9`) the web Models page stores —
	* consecutive requests, and every retry the recovery policy schedules, take
	* the next configured key.
	*/
	const rotateApiKey = createApiKeyRotation();
	const resolveApiKey = async (provider, profile) => {
		const ref = profile.apiKeyEnv;
		if (ref === void 0) return void 0;
		const credentials = ctx.get("credentials");
		const resolveOne = async (candidateRef) => {
			if (credentials !== void 0) {
				const hit = await credentials.resolve(candidateRef);
				return hit !== void 0 && hit.value.length > 0 ? hit.value : void 0;
			}
			const ambient = launchEnvironmentOf(ctx).get(candidateRef);
			return ambient !== void 0 && ambient.value.length > 0 ? ambient.value : void 0;
		};
		const rotated = await rotateApiKey(ref, resolveOne, (candidateRef, value) => assertUsableApiKey(value, "llm-pi-ai", candidateRef), (candidateRef, error) => {
			ctx.logger.warn("llm-pi-ai: skipping an unusable API key from %s during rotation", candidateRef);
	...
	ensureDirectory();
	/** Host-owned request inputs for discovery and probing of one configured route. */
	const storedDiscoveryProfile = (provider) => {
		...
		const defaults = catalogModels(provider);
		const sharedApi = defaults.size === 0 ? void 0 : sharedCatalogApi(defaults);
		...
		return {
			...
			baseURL: profile.baseURL ?? catalogBaseURL,
			api: profile.api ?? sharedApi
		};
	};
	...
	ctx.llm.registerModelProbe(NS, (request, signal) => probeModel({
		...request,
		...signal === void 0 ? {} : { signal }
	}, () => storedDiscoveryProfile(request.provider)));
	let registration;
