...
### The System prompt and Notification sounds sections

The shell registers two standalone sections of its own. **System prompt** edits the custom prompt persisted in the Host `system-prompt` settings namespace, appended after the built-in prompt for every session. **Notification sounds** configures one mode per task event — completed, error, and needs-approval — with a built-in chime, user-picked audio, or mute; custom audio stays in the current browser only.
