...
`dsh-client-ui-settings-general` 是 dsh Web 客户端的设置外壳：Settings 面板从侧边栏底部的控件打开，该控件旁的连接故障指示器提供即时恢复操作；导航由各功能贡献的分区构建；首次运行的用户一次只走一个引导步骤。它还注册设置页面上所有不属于单一功能的内容：触发器、标题栏与关闭控件界面框架、「本地配置文件」操作、「通用」分区及其 `settings.general.item` slot、独立的「系统提示词」与「提示音」分区，以及 `settings` 字典。归具体功能所有的行（「权限」、「语言」、「外观」）、分区（「模型」）与条件式首次使用引导步骤仍由各自的功能包提供；外壳本身不自带任何引导文案。
...
外壳自带两个独立分区。**系统提示词**编辑持久化到 Host `system-prompt` 设置命名空间的自定义提示词，追加在内置提示词之后、对所有会话生效；**提示音**为任务完成、任务报错、任务需要提权三种事件各配置一种模式——默认音、自定义音频或静音，自定义音频仅保存在当前浏览器。
