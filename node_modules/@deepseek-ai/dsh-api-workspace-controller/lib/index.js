...
var WorkspaceCommands = class {
	...
	*/
	...
	/**
	* Permanently delete one cold Session and its stored artifact.
	* @param request - Session identity to delete.
	* @returns deletion acknowledgement.
	*/
	async deleteSession(request) {
		const sessions = this.ctx.get("sessions");
		if (sessions?.get(request.sessionId) !== void 0) throw new RemoteError("session/attached", "A live session instance must be closed before deletion.", { sessionId: request.sessionId });
		let inspection;
		try {
			inspection = await this.ctx.sessionPersistence.inspect(request.sessionId);
		} catch (error) {
		...
		if (location === void 0 || typeof location.path !== "string") throw new RemoteError("session/delete-unsupported", "The active session persistence backend does not expose deletable artifacts.", { sessionId: request.sessionId });
		try {
			await unlink(location.path);
		} catch (error) {
			if (error?.code !== "ENOENT") throw new RemoteError("session/delete-failed", `Unable to delete the session artifact: ${error.message}`, { sessionId: request.sessionId }, { cause: error });
		}
		await this.ctx.workspaceRegistry.unarchiveSession(request.sessionId);
...
let WorkspaceController = (() => {
	...
	return class WorkspaceController extends _classSuper {
		...
		static inject = ["typert", "workspaceRegistry", "sessions", "sessionPersistence"];
		...
		*/
		...
		/**
		* Permanently delete one cold Session and its stored artifacts.
		* @param request - Session identity to delete.
		* @returns deletion acknowledgement.
		*/
