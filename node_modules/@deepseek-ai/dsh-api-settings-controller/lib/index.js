...
const getRequestSchema = z.object({ ref: credentialRefSchema });
...
let CredentialsController = (() => {
	...
	return class CredentialsController extends _classSuper {
		...
		*/
		...
		/**
		* Read one reference's stored value back for this configuration surface. This
		* is the one deliberate secret read path: the Models page shows the key the
		* user saved on this Host, and the answer only ever carries what the caller's
		* own reference resolves to.
		* @param ref - reference name to read.
		* @returns whether a value resolves, its source layer, and the value itself.
		* @throws RemoteError when the request is invalid, no credential provider is mounted, or the provider refuses the read.
		*/
		async get(ref) {
			const request = parseRequest("credentials.get", getRequestSchema, { ref });
			const branded = credentialRef(request.ref);
			const credentials = this.provider();
			if (typeof credentials.resolve !== "function") throw new RemoteError("gateway/internal", "the mounted credential provider cannot resolve references", {});
			try {
				const resolved = await credentials.resolve(branded);
				return resolved === void 0 ? { configured: false } : {
					configured: true,
					...resolved.source === void 0 ? {} : { source: resolved.source },
					value: resolved.value
				};
			} catch (error) {
				throw new RemoteError("credential/rejected", error instanceof Error ? error.message : String(error), { ref: request.ref }, { cause: error });
			}
		}
		/**
