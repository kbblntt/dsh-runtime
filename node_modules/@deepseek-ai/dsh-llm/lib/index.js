...
/** How many numbered companion slots rotation discovers after the primary reference (`<ref>_2` … `<ref>_9`). */
const API_KEY_ROTATION_SLOTS = 8;
/**
* Build a stateful round-robin API-key resolver for one adapter.
*
* The web Models page can store extra keys for a provider under numbered
* companion references (`<ref>_2` … `<ref>_9`, the same layering the primary
* reference resolves through). Every call resolves the primary and each
* companion, then returns the next candidate in rotation, so consecutive
* requests — and every retry a recovery policy schedules — spread across the
* configured keys instead of wearing one out. A candidate that fails the
* usability assertion is skipped for this call, with `onSkip` informed, so one
* malformed stored key degrades the pool instead of failing the request; when
* every candidate fails, the first refusal is rethrown. Zero candidates return
* `undefined` for the adapter's own missing-credential error; exactly one
* candidate behaves exactly like a direct {@link assertUsableApiKey} pass, so
* rotation's observable behavior is reserved for the multi-key case.
*
* The rotation counter is in-memory and per primary reference: restarting the
* host process restarts the cycle, which changes nothing about correctness.
* Candidates are read fresh on every call, so a key added or removed on the
* Models page joins or leaves the cycle at the next request without any reload.
* @returns the stateful resolver taking `(ref, resolveOne, assertUsable, onSkip)`.
*/
function createApiKeyRotation() {
	const counters = /* @__PURE__ */ new Map();
	return async function resolveRotatedApiKey(ref, resolveOne, assertUsable, onSkip) {
		const candidates = [];
		const primary = await resolveOne(ref);
		if (primary !== void 0 && primary.length > 0) candidates.push([ref, primary]);
		for (let slot = 2; slot <= 1 + API_KEY_ROTATION_SLOTS; slot++) {
			const companionRef = `${ref}_${slot}`;
			const value = await resolveOne(companionRef);
			if (value !== void 0 && value.length > 0) candidates.push([companionRef, value]);
		}
		if (candidates.length === 0) return void 0;
		if (candidates.length === 1) return assertUsable(candidates[0][0], candidates[0][1]);
		const start = (counters.get(ref) ?? 0) % candidates.length;
		let refusal;
		for (let at = 0; at < candidates.length; at++) {
			const [candidateRef, value] = candidates[(start + at) % candidates.length];
			try {
				const usable = assertUsable(candidateRef, value);
				counters.set(ref, start + at + 1);
				return usable;
			} catch (error) {
				refusal ??= error;
				onSkip?.(candidateRef, error);
			}
		}
		throw refusal;
	};
...
let LlmRuntime = (() => {
	...
	let _remoteProbeModel_decorators;
	return class LlmRuntime extends _classSuper {
		static {
			...
			_remoteProbeModel_decorators = [Remote("probeModel")];
			...
			__esDecorate(this, null, _remoteProbeModel_decorators, {
				kind: "method",
				name: "remoteProbeModel",
				static: false,
				private: false,
				access: {
					has: (obj) => "remoteProbeModel" in obj,
					get: (obj) => obj.remoteProbeModel
				},
				metadata: _metadata
			}, null, _instanceExtraInitializers);
		...
		*/
		...
		/**
		* Offer to send one minimal completion probe on behalf of the settings
		* namespace this plugin owns. The namespace is the key so a configuration
		* surface can test a draft the same way discovery does: the request names a
		* model plus the endpoint and one-shot credential to reach it with.
		* Disposed with the fiber.
		* @param settingsNs - the namespace whose profiles this probing serves.
		* @param probe - performs one minimal completion and must honor the supplied signal.
		* @returns the disposer that withdraws the offer.
		*/
		registerModelProbe(settingsNs, probe) {
			const dispose = this.ctx.effect(function* () {
				if (settingsNs.length === 0) throw new LlmError("model probe needs a non-empty settings namespace", "INVALID_PROBE");
				if (this.probes.has(settingsNs)) throw new LlmError(`model probe for "${settingsNs}" is already registered`, "DUPLICATE_PROBE");
				this.probes.set(settingsNs, probe);
				yield () => {
					this.probes.delete(settingsNs);
				};
			}.bind(this), "llm.registerModelProbe()");
			return () => void dispose();
		}
		/**
		* Send one minimal completion probe against a draft endpoint and model. Like
		* discovery, the request describes a draft, not a stored route, so nothing
		* here reads or writes settings or credentials — the caller owns both. The
		* probe is deliberately tiny (a one-token reply) so a connectivity check
		* costs essentially nothing.
		* @param settingsNs - namespace whose registered probe serves this draft.
		* @param request - the model id, endpoint, protocol, and one-shot credential to use.
		* @param signal - caller cancellation.
		* @returns the round-trip latency and the probed model id.
		*/
		async probeModel(settingsNs, request, signal) {
			const probe = this.probes.get(settingsNs);
			if (probe === void 0) throw new LlmError(`no model probe is registered for "${settingsNs}"`, "NO_PROBE");
			if (typeof request.model !== "string" || request.model.length === 0) throw new LlmError("model probe needs a model id", "INVALID_PROBE");
			if ((request.provider ?? "").length === 0 && (request.baseURL ?? "").length === 0) throw new LlmError("model probe needs a provider route or a baseURL", "INVALID_PROBE");
			const startedAt = Date.now();
			signal === void 0 ? await probe(request) : await probe(request, signal);
			return {
		...
		/**
		* Remote adapter for one draft model connectivity probe.
		* @param settingsNs - namespace whose registered probe serves this draft.
		* @param request - model id, endpoint, protocol, and one-shot credential to use.
		* @param signal - caller cancellation supplied by the Remote carrier.
		* @returns the round-trip latency and the probed model id.
		* @throws RemoteError with `llm/model-probe-rejected` when the probe refuses or fails.
		*/
		async remoteProbeModel(settingsNs, request, signal) {
			try {
				return await this.probeModel(settingsNs, request, signal);
			} catch (error) {
				throw new RemoteError("llm/model-probe-rejected", error instanceof Error ? error.message : String(error), {
					settingsNs,
					...request.baseURL === void 0 ? {} : { baseURL: request.baseURL },
