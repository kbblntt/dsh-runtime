	...
	factory: (require) => {
		...
		*/
		...
		const zh = {
			...
			"background.description": "选择一张本地图片作为界面背景。面板不透明度决定壁纸透出多少（输入框会自动更实一点），遮罩强度决定整体压暗多少；图片只保存在当前浏览器（localStorage），不会写入 settings.yaml。",
			"background.choose": "选择图片",
			"background.replace": "更换图片",
			"background.clear": "移除背景",
			"background.panel": "面板不透明度",
			"background.scrim": "遮罩强度",
			"background.empty": "未设置自定义背景",
			"background.tooLarge": "图片处理后仍超过 3 MB，请换一张更小的图片。",
			"background.unsupported": "请选择 PNG、JPG、WebP 或 GIF 图片。",
			"background.failed": "背景图片保存失败：浏览器存储空间不足。"
		};
		...
		const en = {
			...
			"background.description": "Pick a local picture as the app background. Panel opacity decides how much wallpaper shows through (the composer stays a little more solid), and the scrim dims the whole picture; the picture lives in this browser (localStorage) and is never written to settings.yaml.",
			"background.choose": "Choose image",
			"background.replace": "Replace image",
			"background.clear": "Remove background",
			"background.panel": "Panel opacity",
			"background.scrim": "Scrim",
			"background.empty": "No custom background",
			"background.tooLarge": "The picture is still over 3 MB after processing; choose a smaller one.",
			"background.unsupported": "Choose a PNG, JPG, WebP, or GIF image.",
			"background.failed": "The background could not be stored: this browser's storage is full."
		};
		//#endregion
		//#region lib/types/client/Background.js
		/**
		* Custom image background: one local picture painted behind the app shell.
		* It is a browser-local cosmetic preference (localStorage), not a settings
		* value: a multi-megabyte data URL has no place in settings.yaml, and the
		* picture is per-browser by nature. The picture and its optional scrim are
		* real fixed elements at `z-index: -1` (a positioned pseudo-element would
		* paint above the non-positioned app content and dim every glyph), and the
		* surfaces that paint the app canvas become translucent glass so the
		* wallpaper stays visible while text keeps a consistent backdrop.
		*/
		const BACKGROUND_STORAGE_KEY = "dsh.background";
		const BACKGROUND_MAX_LENGTH = 3e6;
		const BACKGROUND_MAX_EDGE = 2560;
		const BACKGROUND_DEFAULT_SCRIM = 0;
		const BACKGROUND_DEFAULT_PANEL = 38;
		const BACKGROUND_CSS_TAG = "@deepseek-ai/dsh-client-ui-theme/Background.module.css";
		const BACKGROUND_CSS = "html.dsh-custom-bg body{--dsw-alias-bg-base:color-mix(in srgb,var(--dsw-alias-bg-layer-1) var(--dsh-bg-panel-chat,38%),transparent) !important;--dsw-specific-sidebar-fill:color-mix(in srgb,var(--dsw-alias-bg-layer-2) var(--dsh-bg-panel-side,48%),transparent) !important;--dsw-specific-input-major:color-mix(in srgb,var(--dsw-alias-bg-layer-2) var(--dsh-bg-panel-input,70%),transparent) !important;background-color:var(--dsw-alias-bg-layer-1) !important}html.dsh-custom-bg #dsh-bg-image{pointer-events:none;position:fixed;inset:0;z-index:-1;background-image:var(--dsh-bg-image);background-position:center;background-size:cover;background-repeat:no-repeat}html.dsh-custom-bg #dsh-bg-scrim{pointer-events:none;position:fixed;inset:0;z-index:-1;background-color:var(--dsw-alias-bg-layer-1);opacity:var(--dsh-bg-scrim,.1)}.dshBackground_root{flex-direction:column;gap:12px;display:flex}.dshBackground_card{border:.5px solid var(--dsw-alias-border-l4);border-radius:16px;flex-direction:column;gap:12px;padding:16px;display:flex}.dshBackground_title{font-size:16px;font-weight:500;line-height:24px}.dshBackground_description{color:var(--dsw-alias-label-tertiary);margin:0;font-size:13px;line-height:20px}.dshBackground_preview{border:.5px solid var(--dsw-alias-border-l4);border-radius:12px;height:180px;background-color:var(--dsw-alias-bg-layer-1);background-position:center;background-size:cover;justify-content:center;align-items:center;display:flex;position:relative;overflow:hidden}.dshBackground_previewScrim{pointer-events:none;background-color:var(--dsw-alias-bg-layer-1);position:absolute;inset:0}.dshBackground_empty{color:var(--dsw-alias-label-tertiary);font-size:13px;line-height:20px}.dshBackground_actions{align-items:center;gap:8px;display:flex}.dshBackground_primary{box-sizing:border-box;cursor:pointer;height:32px;color:var(--dsw-alias-label-primary-foreground);background:var(--dsw-alias-button-primary-fill);border:none;border-radius:16px;align-items:center;gap:6px;padding:0 14px;font-family:inherit;font-size:13px;line-height:20px;display:inline-flex}.dshBackground_primary:hover{background:var(--dsw-alias-button-primary-hover)}.dshBackground_secondary{box-sizing:border-box;cursor:pointer;height:32px;color:var(--dsw-alias-label-secondary);background:0 0;border:.5px solid var(--dsw-alias-border-l3);border-radius:16px;align-items:center;padding:0 14px;font-family:inherit;font-size:13px;line-height:20px;display:inline-flex}.dshBackground_secondary:hover{background:var(--dsw-alias-interactive-bg-hover)}.dshBackground_slider{align-items:center;gap:10px;display:flex}.dshBackground_sliderLabel{color:var(--dsw-alias-label-secondary);flex:none;font-size:13px;line-height:20px}.dshBackground_slider input{flex:1;min-width:0}.dshBackground_sliderValue{color:var(--dsw-alias-label-tertiary);flex:none;font-size:12px;line-height:18px;min-width:40px;text-align:right}.dshBackground_error{color:var(--dsw-alias-state-error-primary);margin:0;font-size:12px;line-height:18px}";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(BACKGROUND_CSS_TAG) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "@deepseek-ai/dsh-client-ui-theme";
		...
		/** Read the stored preference, ignoring anything malformed. */
		function readBackgroundState() {
			const empty = () => ({
				image: void 0,
				scrim: BACKGROUND_DEFAULT_SCRIM,
				panel: BACKGROUND_DEFAULT_PANEL
			});
			try {
				...
				const image = typeof parsed.image === "string" && parsed.image.startsWith("data:image/") ? parsed.image : void 0;
				const scrim = typeof parsed.scrim === "number" && parsed.scrim >= 0 && parsed.scrim <= 0.9 ? parsed.scrim : BACKGROUND_DEFAULT_SCRIM;
				const panel = typeof parsed.panel === "number" && parsed.panel >= 0 && parsed.panel <= 100 ? parsed.panel : BACKGROUND_DEFAULT_PANEL;
				return {
					image,
					scrim,
					panel
		...
		/** Persist the preference; false when the browser refuses (quota or private mode). */
		function writeBackgroundState(state) {
			try {
				if (state.image === void 0) localStorage.removeItem(BACKGROUND_STORAGE_KEY);
				else localStorage.setItem(BACKGROUND_STORAGE_KEY, JSON.stringify({
					image: state.image,
					scrim: state.scrim,
					panel: state.panel
				}));
		...
		/**
		* Paint the current preference onto the document (or clear it). The picture
		* and its scrim are real fixed elements at `z-index: -1`, not pseudo-elements:
		* a positioned pseudo-element paints above the non-positioned app content, so
		* a scrim built that way dimmed every glyph instead of the picture behind it.
		*/
		function applyBackgroundState(state) {
			if (typeof document === "undefined") return;
			const root = document.documentElement;
			const existingImage = document.getElementById("dsh-bg-image");
			const existingScrim = document.getElementById("dsh-bg-scrim");
			if (state.image === void 0) {
				root.classList.remove("dsh-custom-bg");
				root.style.removeProperty("--dsh-bg-image");
				root.style.removeProperty("--dsh-bg-scrim");
				root.style.removeProperty("--dsh-bg-panel-chat");
				root.style.removeProperty("--dsh-bg-panel-side");
				root.style.removeProperty("--dsh-bg-panel-input");
				existingImage?.remove();
				existingScrim?.remove();
				return;
			}
			root.classList.add("dsh-custom-bg");
			root.style.setProperty("--dsh-bg-image", `url("${state.image}")`);
			root.style.setProperty("--dsh-bg-scrim", String(state.scrim));
			/* One panel slider drives three surfaces: the transcript stays the most
			transparent, the sidebar a little more solid, and the composer the most
			solid because it holds the text being typed. */
			const panel = Math.max(0, Math.min(100, state.panel));
			root.style.setProperty("--dsh-bg-panel-chat", `${String(Math.round(panel))}%`);
			root.style.setProperty("--dsh-bg-panel-side", `${String(Math.round(Math.min(100, panel + 10)))}%`);
			root.style.setProperty("--dsh-bg-panel-input", `${String(Math.round(Math.min(100, panel + 32)))}%`);
			if (document.body === null) return;
			let image = existingImage;
			let scrim = existingScrim;
			if (image === null) {
				image = document.createElement("div");
				image.id = "dsh-bg-image";
				image.setAttribute("aria-hidden", "true");
			}
			if (scrim === null) {
				scrim = document.createElement("div");
				scrim.id = "dsh-bg-scrim";
				scrim.setAttribute("aria-hidden", "true");
			}
			/* Tree order decides the paint order of two equal-z-index layers: the
			picture first, its scrim over it, both below every app surface. */
			document.body.prepend(image, scrim);
		}
		/**
		* Decode, downscale, and re-encode one picked picture.
		* @param file - the picked image file.
		* @returns the stored data URL, or the copy key of the failure.
		*/
		function normalizeBackgroundFile(file) {
			return new Promise((resolve) => {
				if (!file.type.startsWith("image/")) {
					resolve({ error: "background.unsupported" });
					return;
				}
				...
				image.onload = () => {
					...
					const scale = Math.min(1, BACKGROUND_MAX_EDGE / Math.max(image.naturalWidth, image.naturalHeight, 1));
					...
					if (context === null) {
						resolve({ error: "background.failed" });
						return;
					}
					...
					if (dataUrl.length > BACKGROUND_MAX_LENGTH) dataUrl = canvas.toDataURL("image/jpeg", 0.7);
					if (dataUrl.length > BACKGROUND_MAX_LENGTH) {
						resolve({ error: "background.tooLarge" });
						return;
					}
		...
		/**
		* Render the Background settings page.
		* @param props - slot-delivered injected copy.
		* @returns the page.
		*/
		function BackgroundSection({ t }) {
			if (t === void 0) return null;
			const [state, setState] = (0, react.useState)(() => readBackgroundState());
			const [failure, setFailure] = (0, react.useState)(void 0);
			const inputRef = (0, react.useRef)(null);
			const update = (next) => {
				if (!writeBackgroundState(next)) {
					setFailure(t("background.failed"));
					return;
				}
				setFailure(void 0);
				setState(next);
			...
			const onPick = async (event) => {
				const file = event.target.files === null ? void 0 : event.target.files[0];
				event.target.value = "";
				if (file === void 0) return;
				const result = await normalizeBackgroundFile(file);
				if (result.error !== void 0) {
					setFailure(t(result.error));
					return;
				}
				update({
					image: result.image,
			...
			return (0, react_jsx_runtime.jsx)("div", {
				className: "dshBackground_root",
				children: (0, react_jsx_runtime.jsxs)("div", {
					className: "dshBackground_card",
					children: [(0, react_jsx_runtime.jsx)("span", {
						className: "dshBackground_title",
						children: t("background.title")
					}), (0, react_jsx_runtime.jsx)("p", {
						className: "dshBackground_description",
						children: t("background.description")
					}), (0, react_jsx_runtime.jsxs)("div", {
						className: "dshBackground_preview",
						style: state.image === void 0 ? void 0 : { backgroundImage: `url("${state.image}")` },
						children: [state.image === void 0 ? (0, react_jsx_runtime.jsx)("span", {
							className: "dshBackground_empty",
							children: t("background.empty")
						}) : null, state.image === void 0 ? null : (0, react_jsx_runtime.jsx)("span", {
							className: "dshBackground_previewScrim",
							style: { opacity: state.scrim },
							"aria-hidden": true
						})]
					}), (0, react_jsx_runtime.jsxs)("div", {
						className: "dshBackground_actions",
						children: [(0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: "dshBackground_primary",
							onClick: () => {
								inputRef.current?.click();
							},
							children: state.image === void 0 ? t("background.choose") : t("background.replace")
						}), state.image === void 0 ? null : (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: "dshBackground_secondary",
							onClick: () => {
								update({
									image: void 0,
									scrim: BACKGROUND_DEFAULT_SCRIM,
					...
					}), state.image === void 0 ? null : (0, react_jsx_runtime.jsxs)("label", {
						className: "dshBackground_slider",
						children: [(0, react_jsx_runtime.jsx)("span", {
							className: "dshBackground_sliderLabel",
							children: t("background.panel")
						}), (0, react_jsx_runtime.jsx)("input", {
							type: "range",
							min: "0",
							max: "100",
							step: "2",
							value: String(Math.round(state.panel)),
							"aria-label": t("background.panel"),
							onChange: (event) => {
								update({
									image: state.image,
									scrim: state.scrim,
									panel: Number(event.target.value)
								});
							}
						}), (0, react_jsx_runtime.jsx)("span", {
							className: "dshBackground_sliderValue",
							children: `${String(Math.round(state.panel))}%`
						})]
					}), state.image === void 0 ? null : (0, react_jsx_runtime.jsxs)("label", {
						className: "dshBackground_slider",
						children: [(0, react_jsx_runtime.jsx)("span", {
							...
							max: "90",
							step: "5",
							value: String(Math.round(state.scrim * 100)),
							"aria-label": t("background.scrim"),
							onChange: (event) => {
								update({
									image: state.image,
									scrim: Number(event.target.value) / 100,
									panel: state.panel
								});
							}
						}), (0, react_jsx_runtime.jsx)("span", {
							className: "dshBackground_sliderValue",
							children: `${String(Math.round(state.scrim * 100))}%`
						})]
					}), failure === void 0 ? null : (0, react_jsx_runtime.jsx)("p", {
						className: "dshBackground_error",
						role: "status",
						children: failure
					})]
				})
		...
		*/
		function apply(ctx) {
			...
			applyBackgroundState(readBackgroundState());
			...
			const sectionT = ctx.locale.bind(SETTINGS_NS);
			ctx.slots.inject("settings.section", () => ctx.slots.register({
				name: "settings.section",
				id: "background",
				order: 15,
				label: () => sectionT("background.nav"),
				inject: () => ({ t: sectionT })
			}, BackgroundSection));
		}
		//#endregion
